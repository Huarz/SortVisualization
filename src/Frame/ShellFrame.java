...
public final class ShellFrame extends SortFrame {
        ...
        public void startSorting() {
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() {
                    for (int gap = array.size() / 2; gap > 0 && sorting; gap /= 2) {
                        for (int i = gap; i < array.size() && sorting; i++) {
                            currentComparisonIndex1 = i - gap;
                            currentComparisonIndex2 = i;
                            comparisonCount++;
                            if(array.get(currentComparisonIndex1)>array.get(currentComparisonIndex2))
                            {
                                int temp = array.get(currentComparisonIndex1);
                                array.set(currentComparisonIndex1, array.get(currentComparisonIndex2));
                                array.set(currentComparisonIndex2, temp);
                            }
                                repaint(); // 绘制图形
                                updateLabels();
                                try {
                    ...
                    for (int j = 1; j <= array.size() - 1 && sorting; j++) {
                ...
                protected void done() {
                    ...
                    if (sorting) { // 检查是否是正常完成排序
                        ...
                        JOptionPane.showMessageDialog(ShellFrame.this, "排序已经完成!", "提示", JOptionPane.INFORMATION_MESSAGE);
                    }
                }
            }.execute();
        }
